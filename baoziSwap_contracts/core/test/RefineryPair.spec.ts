...
describe('BaoziPair', () => {
  ...
  const optimisticTestCases: BigNumber[][] = [
    ...
    [1, 5, 5, '1003009027081243732'] // given amountOut, amountIn = ceiling(amountOut / .998)
  ].map(a => a.map(n => (typeof n === 'string' ? bigNumberify(n) : expandTo18Decimals(n))))
  optimisticTestCases.forEach((optimisticTestCase, i) => {
    it(`optimistic:${i}`, async () => {
      ...
      await expect(pair.swap(outputAmount.add(1), 0, wallet.address, '0x', overrides)).to.be.revertedWith('Baozi: K')
  ...
  it('swap:gas', async () => {
    ...
    expect(receipt.gasUsed).to.eq(71980)
  })
  ...
  it('price{0,1}CumulativeLast', async () => {
    ...
    expect(await pair.price0CumulativeLast()).to.eq(initialPrice[0], 'Er1')
    expect(await pair.price1CumulativeLast()).to.eq(initialPrice[1], 'Er2')
    ...
    expect(await pair.price0CumulativeLast()).to.eq(initialPrice[0].mul(10), 'Er3')
    expect(await pair.price1CumulativeLast()).to.eq(initialPrice[1].mul(10), 'Er4')
    expect((await pair.getReserves())[2]).to.eq(blockTimestamp + 10, 'Er5')
    await mineBlock(provider, blockTimestamp + 20)
    await pair.sync(overrides)
    const newPrice = encodePrice(expandTo18Decimals(6), expandTo18Decimals(2))
    expect(await pair.price0CumulativeLast()).to.eq(initialPrice[0].mul(10).add(newPrice[0].mul(10)), 'Er6')
    expect(await pair.price1CumulativeLast()).to.eq(initialPrice[1].mul(10).add(newPrice[1].mul(10)), 'Er7')
    expect((await pair.getReserves())[2]).to.eq(blockTimestamp + 20, 'Er8')
  }).retries(3) // unstable
